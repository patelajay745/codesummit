...
export interface addProblemFormDataTypes {
  title: string;
  description: string;
  constraints: string;
  testCases: testCase[];
...
const sampledpData: addProblemFormDataTypes = {
...
function AddProblem() {
  ...
  const {
    ...
    watch,
    control,
  } = useForm<addProblemFormDataTypes>({
  ...
  useEffect(() => {
    if (authUser?.role !== "ADMIN") navigate({ to: "/dashboard" });
  });

  const onSubmit = (data: addProblemFormDataTypes) => {
    console.log(data);
  };
